...
contract CODE7Token is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 CODE-WETH LP -> CODE pool
    // uint256 public constant INITIAL_CODE_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the CODE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Code7 Finance", "CODE7") {
        // Mints 1 CODE to contract creator for initial pool setup
    ...
    function _getCodePrice() internal view returns (uint256 _CodePrice) {
        try IOracle(codeOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Code: failed to fetch CODE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _CodePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_CodePrice >= taxTiersTwaps[tierId]) {
    ...
    function setCodeOracle(address _CodeOracle) public onlyOperatorOrTaxOffice {
        require(_CodeOracle != address(0), "oracle address cannot be 0 address");
        codeOracle = _CodeOracle;
    }
    ...
    /**
     * @notice Operator mints CODE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CODE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCodePrice = _getCodePrice();
            currentTaxRate = _updateTaxRate(currentCodePrice);
            if (currentCodePrice < burnThreshold) {
                burnTax = true;
            }
        }
