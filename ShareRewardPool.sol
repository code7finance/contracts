...
contract SevenShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accSevenSharePerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.00578703 ether; // 60000 7share / (120 days * 24h * 60min * 60s)
    uint256 public runningTime = 120 days; // 120 days
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SevenShareRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "SevenShareRewardPool: existing pool?");
        }
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accSevenSharePerShare = pool.accSevenSharePerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _sshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accSevenSharePerShare = accSevenSharePerShare.add(_sshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accSevenSharePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _sshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accSevenSharePerShare = pool.accSevenSharePerShare.add(_sshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accSevenSharePerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeSevenShareTransfer(_sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accSevenSharePerShare).div(1e18);
    ...
    // Safe sshare transfer function, just in case if rounding error causes pool to not have enough tSHAREs.
    function safeSevenShareTransfer(address _to, uint256 _amount) internal {
